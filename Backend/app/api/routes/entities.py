...
#     if not document:
#         raise HTTPException(status_code=404, detail="Document not found. Run /summarize first.")

#     try:
#         entities = await run_in_threadpool(extract_entities_from_chunks, document["chunks"])
#         db.update_document(doc_id, {"entities": entities})
#         return {"doc_id": doc_id, "entity_count": len(entities), "entities": entities}
