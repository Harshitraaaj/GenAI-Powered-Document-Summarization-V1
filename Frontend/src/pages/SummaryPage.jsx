...
const SummaryPage = () => {
  ...
  // ── Invalid chunks — read from backend coverage_details ──
  // Backend sends: metadata.coverage_details.missing_chunk_ids = [3, 17, ...]
  // invalid_count = how many chunks failed to summarize
  const missing_chunk_ids = metadata.coverage_details?.missing_chunk_ids || [];
  const invalid_count     = missing_chunk_ids.length;
  ...
  return (
    <div className="max-w-4xl animate-fadeUp">
      ...
      {/* ── Invalid chunks badge — only shown when > 0 ── */}
      {invalid_count > 0 && (
        <div className="flex items-center gap-2 mb-5">
          <span className="text-[11px] font-medium text-rose bg-rose/10 border border-rose/20 rounded-full px-3 py-1">
            {invalid_count} Invalid {invalid_count === 1 ? "Chunk" : "Chunks"}
          </span>
          <span className="text-[11px] text-dim">
            Chunk {missing_chunk_ids.join(", ")} could not be summarized
          </span>
        </div>
      )}
